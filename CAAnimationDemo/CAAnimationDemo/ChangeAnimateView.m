...
static const CGFloat kStep3Duration = 3.0;
static const CGFloat kStep4Duration = 3.0;
...
@implementation ChangeAnimateView
...
- (CAShapeLayer *)topLayer
{
    if (!_topLayer) {
        ...
        _topLayer.strokeColor = [UIColor grayColor].CGColor;
        _topLayer.contentsScale = [UIScreen mainScreen].scale;
        _topLayer.lineCap = kCALineCapRound;
        _topLayer.lineWidth = lineHeight;
        
        UIBezierPath *bezierPath = [UIBezierPath bezierPath];
        [bezierPath moveToPoint:CGPointMake(0, 0)];
        [bezierPath addLineToPoint:CGPointMake(-lineWidth, 0)];
        _topLayer.path = bezierPath.CGPath;
...
- (CAShapeLayer *)bottomLayer
{
    if (!_bottomLayer) {
        ...
        _bottomLayer.lineWidth = lineHeight;
...
- (CAShapeLayer *)centerLayer
{
    if (!_centerLayer) {
        ...
        _centerLayer.lineWidth = lineHeight;
        _centerLayer.lineCap = kCALineCapRound;
...
- (void)startAnimation
{
    ...
    [self.changedLayer removeAllAnimations];
    [self initLayers];
    [self animateStep1];
...
- (void)animateStep2
{
    ...
    //1>.拉伸动画
    CABasicAnimation *stokeAnimation = [CABasicAnimation animationWithKeyPath:@"strokeEnd"];
    stokeAnimation.fromValue = [NSNumber numberWithFloat:0.4];
    stokeAnimation.toValue = [NSNumber numberWithFloat:0.8];
    
    
    //2>.右位移动画
    CABasicAnimation *positionAnimation = [CABasicAnimation animationWithKeyPath:@"transfrom.translation.x"];
    positionAnimation.fromValue = [NSNumber numberWithFloat:-10];
    positionAnimation.toValue = [NSNumber numberWithFloat:0.2*lineWidth];
...
-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    ...
    if ([[anim valueForKey:@"animationName"] isEqualToString:@"animationStep1"]) {
        ...
        [translationAnimation setValue:@"animationStep5" forKey:@"animationName"];
        [_changedLayer addAnimation:translationAnimation forKey:nil];
    }else if ([[anim valueForKey:@"animationName"] isEqualToString:@"animationStep5"]) {
        [self startAnimation];
    }
}
